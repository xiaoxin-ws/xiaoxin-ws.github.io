...
class MetingJSElement {
    ...
    _init() {
        let q = window.document.getElementsByName("meting")[0]
        let config = {}
        for (let i = 0; i < q.attributes.length; i += 1) {
            config[this._camelize(q.attributes[i].name)] = q.attributes[i].value
        }
        let keys = [
    ...
    _loadPlayer(data) {
        ...
        window.document.getElementsByName("meting")[0].appendChild(div)
...
// }
if (window.document.getElementsByName("meting")[0]) {
    let a = new MetingJSElement();
    a._init()
    a._parse()
    // window.document.getElementsByName("meting")[0].appendChild(a.aplayer)
